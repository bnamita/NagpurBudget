...
// loading data var from data.js instead
...
// Namita
var fontSize = Math.min(width,height)/32,
    totalSize = 0;
...
var arcs = burst_group.selectAll("path.ark")
    ...
    .on('mouseover', function(d) {
        if (d.depth > 0) {
            ...
            fade(arcs, 0.4, names, 'name');  // changed fade-to value from 0.1
            update_crumbs(d);
            // Namita
            show_percentages(d)
        }
...
totalSize = arcs.node().__data__.value;

// Namita
function show_percentages(d) {
    ...
    d3.select("#category")
        ...
        .style("font-size", fontSize)
    d3.select("#amount")
        .text("(" +d.value + " of Total)")
        .style("font-size", fontSize)
    d3.select("#explanation")
        .style("visibility", "")
        .style("z-index", "2")
        .style("top", window.innerHeight/1.6 + "px")
...
//NIKHIL : OKk! Here is where I can get totals...
